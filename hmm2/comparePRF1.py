...
def main():
    # Directory paths
    output_dir = 'english/mid_created(0.3)/'
    ...
    for common_name in set(output_files) & set(expected_files):
        ...
        print(f"File: {common_name}")
        print(f"Pitch Precision: {precision_pitch:.2f}, Recall: {recall_pitch:.2f}, F1-score: {fscore_pitch:.2f}")
        print(f"Onset Precision: {precision_onset:.2f}, Recall: {recall_onset:.2f}, F1-score: {fscore_onset:.2f}")
        print()
    ...
    plt.subplot(2, 3, 1)
    plt.hist(precisions_pitch, bins=10, color='palevioletred', edgecolor='black')
    plt.xlabel('Precision (Pitch)')
    plt.ylabel('Frequency')
    plt.title('Precision Histogram (Pitch)')

    plt.subplot(2, 3, 2)
    plt.hist(recalls_pitch, bins=10, color='lightcoral', edgecolor='black')
    plt.xlabel('Recall (Pitch)')
    plt.ylabel('Frequency')
    plt.title('Recall Histogram (Pitch)')

    plt.subplot(2, 3, 3)
    plt.hist(fscores_pitch, bins=10, color='pink', edgecolor='black')
    ...
    plt.subplot(2, 3, 4)
    plt.hist(precisions_onset, bins=10, color='palevioletred', edgecolor='black')
    plt.xlabel('Precision (Onset)')
    plt.ylabel('Frequency')
    plt.title('Precision Histogram (Onset)')

    plt.subplot(2, 3, 5)
    plt.hist(recalls_onset, bins=10, color='lightcoral', edgecolor='black')
    plt.xlabel('Recall (Onset)')
    plt.ylabel('Frequency')
    plt.title('Recall Histogram (Onset)')

    plt.subplot(2, 3, 6)
    plt.hist(fscores_onset, bins=10, color='pink', edgecolor='black')
